...
2. **Service layer** (`src/services/`): All business logic. Plain async functions (not classes, except `GmailClient`).
   ...
   - `ai/` — `aiService.ts` provides thread summaries, smart replies, AI compose, text transform, auto-categorization, smart label classification, and task extraction. `providerManager.ts` manages five providers (`providers/claudeProvider.ts`, `providers/openaiProvider.ts`, `providers/geminiProvider.ts`, `providers/copilotProvider.ts`, and `providers/ollamaProvider.ts` for local/self-hosted OpenAI-compatible servers — Ollama, LM Studio, vLLM — with an optional Bearer token and `listOllamaModels()` to probe `GET /v1/models`). `askInbox.ts` enables natural language inbox queries. `categorizationManager.ts` auto-sorts threads into Primary/Updates/Promotions/Social/Newsletters. `writingStyleService.ts` analyzes user writing style from sent emails and generates auto-draft replies. `taskExtraction.ts` extracts tasks from email threads via AI. `errors.ts` and `types.ts` define shared AI types. Results cached locally via `db/aiCache.ts`.
...
- **Local/self-hosted AI**: The `ollama` provider talks to any OpenAI-compatible server via the OpenAI SDK at `<serverUrl>/v1`. `serverUrl` is normalized (a trailing `/v1` is stripped before re-appending) and the optional `ollama_api_key` secret is sent as a Bearer token. Remote servers on non-default ports (e.g. `:8080`) require the `http(s)://*:*` entries in `capabilities/default.json` — a portless URLPattern scope only matches the default port (80/443), so without the `:*` wildcard the Tauri http plugin rejects the request with "url not allowed on the configured scope". Capability changes require a Rust rebuild (HMR won't pick them up)
